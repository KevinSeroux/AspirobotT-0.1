...
public class AgentExploPathMultiObj extends Robot{


    public AgentExploPathMultiObj(Environment env) {
        super(env);
    }
    ...
    private LinkedList<Action> explorationLargeur(MentalState m) throws explorationLargeurNotFoundException {
    ...
    private Collection<? extends Noeud> expand(Noeud node) {
        ...
        for (Action a:possibleActionsByPosition(node.getEnvironnement(),node.getPositionRobot())) {
            ...
            s = new Noeud(node,node.getEnvironnement(),node.getPathCost() + 1,node.getProfondeur()+1, futurePosition, performance); //TODO remplacer 1 par Action.getCoutAction()
